...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + ControlFreak2.CF2Input.GetAxis("Mouse X") * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * ControlFreak2.CF2Input.GetAxis("Mouse Y");
            }
            else
            {
                // Inverted Y
        ...
        if (enableZoom)
        {
            ...
            if(ControlFreak2.CF2Input.GetKeyDown(zoomKey) && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                {
                    isZoomed = true;
            ...
            {
                if(ControlFreak2.CF2Input.GetKeyDown(zoomKey))
                {
                    isZoomed = true;
                }
                else if(ControlFreak2.CF2Input.GetKeyUp(zoomKey))
                {
                    isZoomed = false;
                }
            }
        ...
        if(enableJump && ControlFreak2.CF2Input.GetKeyDown(jumpKey) && isGrounded)
        {
            Jump();
        }
        ...
        if (enableCrouch)
        {
            if(ControlFreak2.CF2Input.GetKeyDown(crouchKey) && !holdToCrouch)
            {
                Crouch();
            }
            ...
            else if(ControlFreak2.CF2Input.GetKeyUp(crouchKey) && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector3 targetVelocity = new Vector3(ControlFreak2.CF2Input.GetAxis("Horizontal"), 0, ControlFreak2.CF2Input.GetAxis("Vertical"));
            ...
            if (enableSprint && ControlFreak2.CF2Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
